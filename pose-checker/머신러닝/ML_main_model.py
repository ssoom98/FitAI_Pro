...
train_df = pd.concat([pd.read_csv('D:/KHH/team_project/ML_data/ML_X_data.csv'),folder_type_df['type']], axis=1, ignore_index=True)

train_df.columns = ['folder', 'key_point', 'type']
...
save_path = "D:/KHH/team_project/proseced_logic/ML_main_model/"
eval_save_path = "D:/KHH/team_project/proseced_logic/ML_main_model/model_evaluation/"
results.to_csv(eval_save_path+"main_model_evaluation_results.csv", index=False)
print("모델 평가 결과가 'model_evaluation_results.csv' 파일에 저장되었습니다.")

joblib.dump(xgb_model, save_path+"main_xgb_model.pkl")
joblib.dump(lgbm_model, save_path+"main_lgbm_model.pkl")