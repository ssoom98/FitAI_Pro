...
folder_type_df = ML_df.groupby("folder")["type"].unique().reset_index()

train_df = pd.concat([pd.read_csv('D:/KHH/team_project/ML_data/ML_X_data.csv'),folder_type_df['type'] ,folder_workout_df['workout']], axis=1, ignore_index=True)

train_df.columns = ['folder', 'key_point', 'type', 'workout']

train_df['type'] = train_df['type'].map(lambda x : x[0])
...
for tp in type_list:
    train_data_df = train_df[train_df['type'] == tp]
    X = np.array(train_data_df['key_point'].to_list())
    y = np.array(train_data_df['workout'])
    ...
    if '/' in tp :
        tp = tp.replace('/','_')
    save_path = "D:/KHH/team_project/proseced_logic/ML_sub_model/"
    eval_save_path = "D:/KHH/team_project/proseced_logic/ML_sub_model/model_evaluation/"
    results.to_csv(eval_save_path+tp+"_model_evaluation_results.csv", index=False)
    print("모델 평가 결과가 'model_evaluation_results.csv' 파일에 저장되었습니다.")

    joblib.dump(xgb_model, save_path+tp+"_xgb_model.pkl")
    joblib.dump(lgbm_model, save_path+tp+"_lgbm_model.pkl")